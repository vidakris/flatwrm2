...
__all__ = ['prediction','validation','flatwrm2_from_commandline']
...
def prediction(data_in,  batch=1, verbose=True, DEBUG=False, GPU=0, modelfile="LSTM_weights_keplerSC.h5", save_model=False):
    ...
    modelfile = os.path.join(PACKAGEDIR,modelfile)
    try:
        model = tf.keras.models.load_model(modelfile)
    except OSError:
        print('\033[91m'+'Could not find model file: '+os.path.basename(modelfile)+'  Try using the -m option, or specify an existing model file when calling the function.\nAvailable weight files:'+'\033[0m')
        import glob
        for weight in sorted(glob.glob(os.path.join(PACKAGEDIR,'*.h5'))):
...
def usage():
    print( "Usage: flatwrm2 [options] <input file(s)>")
    print( "Options:")
...
def flatwrm2_from_commandline(args=None):
    """Use flatwrm2 from command-line."""
    import getopt
    import sys
    ...
    for file in files:
        _ = prediction(file, batch=batch, DEBUG=debug, GPU=gpu, save_model=True, modelfile=modelfile)
